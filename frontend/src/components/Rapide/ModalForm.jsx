...
const ModalForm = ({ user, onClose, onCommandeCreated }) => {
  ...
  const validateForm = useCallback(() => {
    const errors = {};
    if (currentStep === 1) {
      if (!formData.nomExp.trim())
        errors.nomExp = "Le nom complet de l'expéditeur est requis.";
      if (!formData.numExp.trim())
        errors.numExp = "Le numéro de téléphone de l'expéditeur est requis.";
      else if (!/^\+?\d{8,}$/.test(formData.numExp.trim()))
        errors.numExp = "Format de numéro de téléphone invalide.";
    }
    if (currentStep === 2) {
      if (!formData.nomDest.trim())
        errors.nomDest = "Le nom complet du destinataire est requis.";
      if (!formData.numDest.trim())
        errors.numDest = "Le numéro de téléphone du destinataire est requis.";
      else if (!/^\+?\d{8,}$/.test(formData.numDest.trim()))
        errors.numDest = "Format de numéro de téléphone invalide.";
      if (!formData.adresseDest.trim())
        errors.adresseDest = "L'adresse complète du destinataire est requise.";
    }
    if (currentStep === 3) {
      if (!formData.nomColis.trim())
        errors.nomColis = "La description du colis est requise.";
      if (formData.nombreColis <= 0)
        errors.nombreColis = "Le nombre de colis doit être supérieur à zéro.";
      if (
        formData.assurance &&
        (formData.valeurColis <= 0 || isNaN(formData.valeurColis))
      ) {
        errors.valeurColis =
          "Veuillez entrer une valeur valide pour le colis si l'assurance est activée.";
      }
    }
    if (currentStep === 4) {
...
J'ai passé une commande (${
      commandeConfirmee.commandeId
    }) à livrer le ${new Date(
      commandeConfirmee.colis.dateLivraison
    ).toLocaleDateString("fr-FR")} à ${commandeConfirmee.colis.heureLivraison}. 
        ...
        <div className="relative w-full h-full flex flex-col items-center justify-center">
          ...
          >
            ...
            <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto bg-black bg-opacity-50 p-2 rounded-lg">
              Merci de votre patience, votre commande sera confirmée dans{" "}
              {adTimer} secondes
            </p>
          </motion.div>
        </div>
                  ...
                  {currentStep === 4 && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-white shadow-lg rounded-xl p-6 space-y-4">
                        <div className="mb-6">
                          <h4 className="text-lg font-bold text-emerald-700 mb-3">
                        ...
                        <div className="mb-6">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <p className="font-medium">
                                {formData.valeurColis
                                  ? `${formData.valeurColis} FCFA`
                                  : "N/A"}
                              </p>
                            </div>
                            <div>
                      ...
                      <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                        <div className="flex items-center">
                          <svg
                            className="w-5 w-5 text-yellow-600 mr-2"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
                          ...
                          <p className="text-yellow-700">
                            Le prix sera fixé par notre équipe et vous sera
                        ...
                        >
                          <input
                            ...
                            className={`form-checkbox h-5 w-5 text-emerald-600 rounded focus:ring-emerald-500 ${
                              formErrors.acceptCGU ? "border-red-500" : ""
                            }`}
                          />
...
const SummarySection = ({ title, children }) => (
  <motion.div variants={itemVariants} className="mb-6">
    ...
    <div className="bg-emerald-50 p-4 rounded-xl">{children}</div>
  </motion.div>
);
const SummaryItem = ({ label, value }) => (
  <motion.div
    variants={itemVariants}
    className="flex justify-between py-2 border-b border-emerald-100 last:border-b-0"
  >
...
const Confirmation = ({ commande, onClose, onNewOrder }) => {
  ...
  const Advertissement = () => (
    ...
    >
      ...
      >
        ...
        >
          <p className="mb-4">Votre commande a été confirmée avec succès.</p>
          <p>
            Merci de faire confiance à <span className="font-bold">NAYA</span>,
            votre partenaire de livraison express.
          </p>
        </motion.div>
      </motion.div>
  ...
  const handleSavePDF = async () => {
    ...
    try {
      ...
      addSection(
        "Expéditeur",
        [
          `${commande.expedition?.prenom || ""} ${
            commande.expedition?.nom || ""
          }`,
      ...
      addSection(
        "Destinataire",
        [
          `${commande.destination?.prenom || ""} ${
            commande.destination?.nom || ""
          }`,
      ...
      addSection(
        "Livraison",
        [
          `Date: ${formatDate(commande.livraison?.date)} à ${formatTime(
            commande.livraison?.heure
          )}`,
          `Instructions: ${commande.livraison?.instructions || "Aucune"}`,
          `Zones: ${
            commande.livraison?.zones?.map((z) => z.displayName).join(", ") ||
            "Aucune"
          } `,
        ].join("\n"),
        1
      ...
      pdf.text(
        "NAYA - Service Livraison Rapide Professionnel",
        105,
        footerY + 5,
        "center"
      );
      pdf.text(
        "contact@naya.ci - (+225) 27 20 30 40 50 - www.naya.ci",
        105,
        footerY + 10,
        "center"
      );
  ...
  return (
    <div className="h-full flex flex-col relative bg-gradient-to-b from-emerald-50 to-white">
      ...
      <div
        className="flex-1 min-h-0 overflow-y-auto p-6 hide-scrollbar"
        id="print-section"
      >
        <motion.div
          initial={{ opacity: 0 }}
              ...
              >
                ...
                <div className="space-y-2 text-lg text-emerald-900">
                  ...
                  <p className="flex items-start">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-5 w-5 mr-2 mt-1 text-emerald-600"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span className="font-bold text-emerald-900">
                            {zone.displayName || zone.quartier || "N/A"}
                          </span>
                        </div>
                        <span className="font-bold text-emerald-600">
                          {zone.price || 0} FCFA
                ...
                <div className="space-y-3 text-lg text-emerald-900">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-2">
                    ...
                    <div>
                      ...
                      <p
                        className={`font-bold ${
                          assurance ? "text-emerald-600" : "text-gray-600"
                        }`}
                      >
                        {assurance ? "Oui" : "Non"}
                      </p>
